...
def split_unit_pieces(out_dir: Path, out_file: Path, num: int):
...
def palettes_out(src: Path, dst: Path):
    ...
    for i in range(16):
        ...
        if not portrait:
            split_unit_pieces(out_dir, out_file, i)


@click.command()
@click.argument('src')
@click.argument('dst')
def paletted_bmp(src, dst):
    src = Path(src)
    dst = Path(dst)
    for src_image in tqdm(list(src.glob('*.bmp'))):
        palettes_out(src_image, dst)
